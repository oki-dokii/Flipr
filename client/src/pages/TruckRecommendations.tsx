...
const TruckRecommendations = () => {
    ...
    const checkExistingBooking = async () => {
        try {
            ...
            if (response.ok) {
                const data = await response.json();
                // Find approved booking for this shipment
                const approvedBooking = data.bookings?.find(
                    (b: any) => b.shipment_id === parseInt(shipmentId!) && b.status === 'approved'
                );
                setExistingBooking(approvedBooking || null);
            }
        } catch (error) {
    ...
    const handleBookingRequest = async (truckId: number, truckName: string) => {
        if (!startDate || !endDate) {
            toast.warning('Please select start and end dates for your booking.');
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                toast.success(`Booking request sent for ${truckName}!`, {
                    description: 'The dealer will review your request.'
                });
            } else {
                ...
                <div className="glass-card p-6 mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input
                                type="date"
                                className="w-full p-2 rounded bg-background border border-white/10"
                                value={startDate}
                    ...
                    <div className="space-y-6">
                        {recommendations.map((rec, index) => (
                            <div key={rec.truck.id} className="glass-card p-6 hover:scale-[1.02] transition-transform">
                                ...
                                {/* Action Button */}
                                {existingBooking ? (
                                    <div className="w-full p-4 bg-green/20 border border-green/30 rounded-lg text-center">
                                        <p className="text-green font-medium">
                                            ✅ Booking Already Approved
                                        </p>
                                        <p className="text-sm text-muted-foreground mt-1">
                                            Truck: {existingBooking.truck_name}
                                        </p>
                                    </div>
                                ) : (
                                    <Button
