...
import { sendBookingApprovedEmail, sendBookingRejectedEmail } from '../services/notificationService.js';
...
router.post('/request', authenticateToken, requireRole('warehouse'), async (req, res) => {
    try {
        const { shipment_id, truck_id, notes, start_date, end_date } = req.body;

        if (!shipment_id || !truck_id || !start_date || !end_date) {
            return res.status(400).json({ error: 'Shipment ID, Truck ID, Start Date, and End Date are required' });
        }
        ...
        // Check strict maintenance status only
        if (truck.availability_status === 'maintenance') {
            return res.status(400).json({ error: 'Truck is under maintenance' });
        }

        // Check for time-based overlaps (approved bookings only)
        const overlap = checkBookingOverlap(truck_id, start_date, end_date);
        if (overlap) {
            return res.status(400).json({
                error: 'Truck is already booked for these dates',
                existingBooking: {
        ...
        // Check for existing booking request from same warehouse (duplicate request)
